...
export default function Nav({ isNavOpen }: { isNavOpen: boolean }) {
  return (
    <motion.div
      ...
      initial="initial"
      className="h-screen overflow-scroll fixed left-0 z-10 w-screen bg-dark-600"
    >
      ...
      <div className="mx-auto -m-[30%] max-w-[1280px] flex space-y-4 flex-col">
        {navList.map((list, key) => {
          return (
            <div
              key={key}
              className="text-gray-600 text-[3rem]  md:text-heading-xl hover:text-primary-600 font-montserrat md:hover:text-[100px]"
            >
              - {list}
            </div>
          );
